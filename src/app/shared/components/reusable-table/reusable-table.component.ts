...
type TableKey<T extends object> = keyof T & string;
@Component({
...
export class ReusableTableComponent<T extends object = object> implements OnInit {
    ...
    @Output() actionClick = new EventEmitter<{ type: string; row: T }>();
    ...
    isPageNumber(page: PageItem): page is number {
    ...
    onActionClick(type: string, row: T): void {
        this.actionClick.emit({ type, row });
    }

    getBadgeClass(value: unknown): string {
        const lowerValue = String(value).toLowerCase();
        if (lowerValue === "active" || lowerValue === "true") return "bg-green-100 text-green-800";
        if (lowerValue === "inactive" || lowerValue === "false" || lowerValue === "blocked")
            return "bg-red-100 text-red-800";
        if (lowerValue === "pending") return "bg-yellow-100 text-yellow-800";
    ...
    getEndItem(): number {
        if (!this.pagination) return 0;
        return Math.min(this.pagination.page * this.pagination.limit, this.pagination.totalItems);
    }

    getPageNumbers(): PageItem[] {
   ...
   getCellValue(row: T, key: TableKey<T>): unknown {
    return row[key];
  }

  getDateValue(row: T, key: TableKey<T>): string | number | Date | null {
    const value = row[key];
