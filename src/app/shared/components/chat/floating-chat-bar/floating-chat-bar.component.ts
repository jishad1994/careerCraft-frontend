...
openChats: MiniChat[] = [];
  ...
  openChat(conversation: Conversation): void {
    ...
    const existingChat = this.openChats.find(
      (c) => c.conversation._id === conversation._id
    );
  ...
  markAsRead(miniChat: MiniChat): void {
    const unreadMessageIds = miniChat.messages
      .filter(
        (msg) =>
          msg.receiverId === this.currentUserId && msg.status !== 'read'
      )
      .map((msg) => msg._id);
 
    if (unreadMessageIds.length > 0) {
      this.socketService.markChatMessagesAsRead(
        miniChat.conversation._id,
        unreadMessageIds
      );
    }
    ...
    const conversation = this.allConversations.find(
      (c) => c._id === miniChat.conversation._id
    );
    if (conversation) {
      const participant = conversation.participants.find(
        (p) => p.userId === this.currentUserId
