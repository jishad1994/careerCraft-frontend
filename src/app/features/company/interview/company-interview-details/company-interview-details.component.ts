...
import { InterviewReturnState, InterviewWithPopulated } from "../../../../models/job-application/job-application.model";
...
export class CompanyInterviewDetailsComponent implements OnInit, OnDestroy {
    ...
    viewApplication(): void {
        if (this.interview) {
            this.router.navigate(["/company/dashboard/applications", this.interview.applicationId]);
        }
    }
    ...
    submitReschedule(): void {
        ...
        const scheduledAt = new Date(`${this.newScheduledDate}T${this.newScheduledTime}`);
    ...
    canJoin(): boolean {
        ...
        const scheduledTime = new Date(this.interview.interview.scheduledAt).getTime();
        const now = Date.now();
    ...
    canEdit(): boolean {
        if (!this.interview) return false;
        return ["scheduled", "rescheduled"].includes(this.interview.interview.status);
    }
