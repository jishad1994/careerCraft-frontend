...
export class CompanyApplicationViewComponent implements OnInit, OnDestroy {
  ...
  viewProfile(): void {
    if (this.application) {
      this.router.navigate(
        ['company/dashboard/candidates', this.application.applicantDetails._id],
        {
          state: {
  ...
  joinVideoInterview(interview: IInterview): void {
    ...
    this.router.navigate([
      '/company/dashboard/video-interview',
      this.applicationId,
      interview.round,
    ]);
  }
  ...
  canJoinInterview(interview: IInterview): boolean {
    ...
    return (
      now >= scheduledTime - fifteenMinutes && interview.status === 'scheduled'
    );
  }
}
