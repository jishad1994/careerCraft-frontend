...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, MatSnackBarModule, ImageCropperComponent],
...
export class BasicProfileComponent implements OnInit, OnChanges, OnDestroy {
    ...
    initializeForm(): void {
        this.basicInfoForm = this.fb.group({
            ...
            address: this.fb.group({
                ...
                postalCode: ["", [Validators.required, Validators.pattern(/^[A-Z0-9\s-]{3,10}$/i)]],
            }),
        });
    }
                ...
                this.userProfileService
                    ...
                    .subscribe({
                        next: (response) => {
                            ...
                            this.resetProfileImageUploadState();
                        },
                        error: (error: { error?: { message?: string } }) => {
                            this.resetProfileImageUploadState();

                            this.snackBar.open(error.error?.message || "Upload failed", "Close", { duration: 3000 });
                        },
                    });
            })
            .catch(() => {
    ...
    private resetProfileImageUploadState(): void {
    ...
    return this.profile?.profilePicture?.location?.trim() || "";
}
